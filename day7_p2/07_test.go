...
func TestNewHand_FiveOfAKind(t *testing.T) {
	hand := NewHand([]int{int(Joker), int(Joker), int(Joker), int(Joker), int(Joker)}, 100)
...
func TestNewHand_ThreeOfAKind(t *testing.T) {
	hand := NewHand([]int{int(Joker), int(Joker), int(Two), int(Queen), int(Ten)}, 100)
...
func TestOrderedByWeakest(t *testing.T) {
	hands := []Hand{
		...
		*NewHand([]int{int(Ten), int(Five), int(Five), int(Joker), int(Five)}, 0),
	}
...
func TestOrderedByWeakest_SameKind_WinsFirstHighCard(t *testing.T) {
	hands := []Hand{
		*NewHand([]int{int(Joker), int(King), int(King), int(King), int(Two)}, 0),
		*NewHand([]int{int(Queen), int(Queen), int(Queen), int(Queen), int(Two)}, 0),
	}

	handsByWeakest := OrderedByWeakest(hands)

	if handsByWeakest[0].Cards[0] != Joker {
...
func TestRun(t *testing.T) {
	...
	if totalWinnings != 5905 {
		t.Fatalf("The total winnings is %d when it should be 5905", totalWinnings)
	}
}
